...
which is quite a different time series (black). We can see in (\ref{eq:recurerror}) that each value $y_{t}$ has its own error $\epsilon_{t}$ but it also carries with it the error of $y_{t-1}$, and the errors in all previous values. This means that the error at time point $t$ will depend on previous errors. In other words: there is \emph{autocorrelation} within the time series of the errors. In Figure~\ref{fig:lintrend3} we have two other time series constructed with (\ref{eq:recurerror}).
...
\begin{figure}
\includegraphics[width=0.7\textwidth]{PTS-cnsterr}
\caption{Residuals }
\label{fig:cnsterr}
\setfloatalignment{b}
\end{figure}

For the time series constructed with equation~\ref{eq:recurerror} we get Figure~\ref{fig:cumerr}. There is clear autocorrelation within the residuals.
...
\begin{figure}
\includegraphics[width=0.8\textwidth]{PTS-cumerr}
