...
p1 <- ggplot(data = lintrend2) +
        ...
        labs(title = "Time series $z_{t}=y_{t}-y_{t-1}$", x = "t", y = "result") +
        JT.theme
...
p2 <- ggplot(data = autocor, aes(x = lag, y = acf)) +
        ...
        scale_x_continuous(limits=c(0, 10), breaks = seq(0,10,1)) +
        scale_y_continuous(limits=c(-1, 1), breaks = seq(-1,1,0.1)) +
        labs(title = "Autocorrelation $z_{t}$", x = "lag", y = "acf") +
        JT.theme
grid.arrange(p1, p2, nrow=1)
@
...
This looks pretty random, and the autocorrelation graph indicates that there is probably no autocorrelation. The mean of the differenced time series is \Sexpr{round(mudiv,3)} and the standard deviation is \Sexpr{round(sddiv,3)}. Our first tentative forecast for the next k time points goes as follows:
\begin{itemize}
...
lintrend2.forecast$y[1] <- lintrend2$ynoiserec[n] + mudiv
