...
\subsection{Making use of autocorrelation}

The presence of autocorrelation within time series seems to be a great nuisance. However, we can make good use of it. When our time series has the underlying structure given by:
\begin{equation}
...
it follows that:
\begin{equation}
	y_{t} - y_{t-1} = b_{1} + \epsilon_{t}(0,\sigma) = \epsilon_{t}(b_{1},\sigma)
\end{equation}

In other words: if a time series has the underlying structure given by equation (\ref{eq:linstruc}), then a new time series constructed by taking the difference of each value $y_{t}$ and its previous value $y_{t-1}$ will be a stationary time series with constant mean $\mu=b_{1}$ and constant standard deviation $\sigma=\sigma$. This knowledge can get us started with making our first forecast!

\newpage
As an example we start with the following time series in Figure~\ref{fig:lintrend4}\sidenote{the black time series from Figure~\ref{fig:lintrend2}}.
...
With some goodwill we could detect a linear trend, which means that the underlying recursive formula associated with this time series could be of the form
\begin{equation}
	y_{t} = y_{t-1} + b_{1} \quad t=1 \ldots n
\end{equation}

We calculate a new time series $z_{t}$ given by:
\begin{equation}
	z_{t} = y_{t} - y_{t-1} \quad t=2 \ldots n
\end{equation}

This is called a \emph{once-differenced}\index{differenced time series}\index{differencing} time series. The values of $z_{t}$ are:
\begin{equation}
	\begin{split}
	  z_{1} &= NA  \\
...
This looks pretty random, and the autocorrelation graph indicates that there is probably no autocorrelation. The mean of the differenced time series is \Sexpr{round(mudiv,3)} and the standard deviation is \Sexpr{round(sddiv,3)}. 
\newpage
Our first tentative forecast (Figure~\ref{fig:lintrend6}) for the next k\sidenote{here k=5} time points goes as follows:
\begin{itemize}
  \item the trendline (red and dashed) is given by $y_{i} = y_{i-1}$ + \Sexpr{round(mudiv,3)}  for i=1, $\ldots$ , n
  \item the starting point of the trendline is the ''intercept" of the linear regression through the time series data
	\item the forecasted values are given by $y_{i} = y_{i-1}$ + \Sexpr{round(mudiv,3)}  for i=(n+1), $\ldots$ , (n+k)
	\item the starting point of the forecast $y_{n+1}$ is equal to the last point of the trendline + \Sexpr{round(mudiv,3)}
	\item the 95\% upper confidence level is given by $y_{i}$ + 1.96*$\sqrt{i-n}$*\Sexpr{round(sddiv,3)}  for i=(n+1), $\ldots$ , (n+k)
	\item the 95\% lower confidence level is given by $y_{i}$ + 1.96*$\sqrt{i-n}$*\Sexpr{round(sddiv,3)}  for i=(n+1), $\ldots$ , (n+k)
\end{itemize}
...
In our example we also decided that there was no autocorrelation remaining in the residuals, while there was some sign of autocorrelation at lag1 and lag2. A second question is: can we build out trend model in such a way that it further reduces serial correlation within the residuals?

\subsection{Finding the trend by smoothing}
