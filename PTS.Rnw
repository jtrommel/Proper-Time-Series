...
ggplot(data = pattern) +
        ...
        scale_x_continuous(limits=c(0, n), breaks = seq(0, n, 2)) +
        scale_y_continuous(limits=c(0, 150), breaks = seq(0, 150, 10)) +
        labs(title = "Non-linear pattern\nfitted trend - quadratic", x = "t", y = "result") +
        JT.theme
@
...
ggplot(data = lintrend2) +
        ...
        geom_point(aes(x = t, y = ynoiserec), size = 2, col = "black") +
        scale_x_continuous(limits=c(0, n), breaks = seq(0,n,1)) +
...
In Figure~\ref{fig:lintrend2} we have a time series with a perfect linear trend (red). This is very unrealistic because we know that any time series is only one of many possible representations, because $Y_{t}$ is a chance variable. So we should add some chance element into the definition of the time series. A standard procedure is to add some noise to each measurement $y_{t}$:

\begin{equation}
	y = b_{0} + b_{1}t + \epsilon \quad with \quad \epsilon=N(0,\sigma)
\end{equation}

and we get the time series in blue. This is close around the trend line.

However, when we build up the time series with the recursion formula \emph{using the same values for $\epsilon$ for each value of $t$}
\begin{equation}
	y_{t+1} = y_{t} + b_{1} + \epsilon_{t+1} \quad with \quad \epsilon_{t+1}=N(0,\sigma)
\end{equation}

we get a quite different time series (black). The reason is that each value $y_{t}$ has its own error $\epsilon_{t}$ but it also carries with it (part of) the error of $y_{t-1}$, and the errors in all previous values. This means that the error at time point $t$ will depend on previous errors. In other words: there is \emph{autocorrelation} within the time series of the errors.
