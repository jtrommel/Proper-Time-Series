...
ggplot(data = autolin, aes(x = lag, y = acf)) +
        ...
        labs(title = "Autocorrelation (linear dependency)", x = "t", y = "result") +
        JT.theme
@
...
ggplot(data = auto, aes(x = lag, y = acf)) +
        ...
        scale_x_continuous(limits=c(0, 10), breaks = seq(0,10,1)) +
        scale_y_continuous(limits=c(0, 1), breaks = seq(0,1,0.1)) +
        labs(title = "Autocorrelation (IID-constructed)", x = "t", y = "result") +
        JT.theme
@
...
ggplot(data = part, aes(x = lag, y = acf)) +
        ...
        scale_x_continuous(limits=c(0, 10), breaks = seq(0,10,1)) +
        scale_y_continuous(limits=c(0, 1), breaks = seq(0,1,0.1)) +
        labs(title = "Partial autocorrelation (linear dependency)", x = "t", y = "result") +
        JT.theme
@
...
In \ref{subsec:autocor} we constructed a time series where each value $y_{t}$ was equal to the previous value $y_{t-1}$ plus a constant:
\begin{equation}
	y_{t}=0.2 + y_{t-1} \quad with \quad y_{1}=5.2
\end{equation}

The use of this recursion formula makes the values of $y_{t}$ \emph{dependent} of each other: there is autocorrelation. The time series is not stationary and it shows a linear pattern (Figure~\ref{fig:lintrend}).
...
ggplot(data = lintrend, aes(x = t, y = y)) +
        ...
        labs(title = "Linear pattern", x = "t", y = "result") +
        JT.theme
@
...
Adding some white noise to the data we get Figure~\ref{fig:lintrendnoise}. We can still see a pattern in the time series created by the recursion formula. In this case the presence of \emph{autocorrelation} creates a linear pattern.

<<label=lintrendnoise, fig=TRUE, include=FALSE, echo=FALSE>>=
lintrend$ynoise <- lintrend$y + rnorm(n, 0, 1)
ggplot(data = lintrend, aes(x = t, y = ynoise)) +
        geom_point(size = 2) +
        scale_x_continuous(limits=c(0, n), breaks = seq(0,n,1)) +
        scale_y_continuous(limits=c(0, 10), breaks = seq(0,10,1)) +
        labs(title = "Linear pattern\ndata with white noise", x = "t", y = "result") +
        JT.theme
@

\begin{marginfigure}[0cm]
\includegraphics[width=1\textwidth]{PTS-lintrendnoise}
\caption{Linear pattern with noise}
\label{fig:lintrendnoise}
\setfloatalignment{b}
\end{marginfigure}

A slight change in the recursion formula (the co\"{e}ffici\"{e}nt of $y_{t-1}$) gives us other patterns: equation~\ref{eq:exp} gives us the non-linear pattern in Figure~\ref{fig:nonlintrendnoise}-left, while equation~\ref{eq:asympt} gives us the non-linear pattern in Figure~\ref{fig:nonlintrendnoise}-right.
\begin{equation}
	y_{t} = 0.2 + 1.2y_{t-1}
\label{eq:exp}
\end{equation}
...
\label{eq:asympt}
\end{equation}
<<label=nonlintrendnoise, fig=TRUE, include=FALSE, echo=FALSE>>=
n <- 20
pattern <- data.frame(t = seq(1, n, 1),
                      y1 = 0,
                      y2 = 0)
pattern$y1 <- -1 + 6*(1.2^pattern$t) + rnorm(n, 0, 4)
pattern$y2 <- 1 + 4*(0.8^pattern$t) + rnorm(n, 0, 0.3)
...
\begin{figure}
\includegraphics[width=0.8\textwidth]{PTS-nonlintrendnoise}
\caption{Non-linear patterns with noise}
\label{fig:nonlintrendnoise}
\setfloatalignment{b}
\end{figure}

\subsection{Finding the trend with linear regression methods}

The general behaviour of $y_{t}$ as a function of $t$ can be described by a function $y=f(t)$ which we call the \emph{general} trend\index{trend}. Linear regression\sidenote{The ''linear" in linear regression refers to the co\"{e}ffici\"{e}nts. The variabel $t$ can appear in a polynomial, in an exponential function ...} can be used to find $f(t)$. 

For example we can fit a quadratic curve (Figure~\ref{fig:polytrend})-left through the datapoints in Figure~\ref{fig:nonlintrendnoise}-left. Not bad, but not great. Maybe it would be better to use an exponential function (Figure~\ref{fig:polytrend}-right):

<<label=polytrend, fig=TRUE, include=FALSE, echo=FALSE>>=
coef <- tidy(lm(y1 ~ t + I(t^2), data = pattern))$estimate
pattern$fity <- coef[1] +
              coef[2]*pattern$t +
              coef[3]*(pattern$t)^2
p1 <- ggplot(data = pattern) +
        ...
        scale_x_continuous(limits=c(0, n), breaks = seq(0, n, 2)) +
        scale_y_continuous(limits=c(0, 150), breaks = seq(0, 150, 10)) +
        labs(title = "Non-linear pattern\nfitted trend - polynomial", x = "t", y = "result") +
        JT.theme
pattern$lny1 <- log(pattern$y1)
coef <- tidy(lm(lny1 ~ t, data = pattern))$estimate
pattern$fityln <- exp(coef[1])*exp(coef[2]*pattern$t)
p2 <- ggplot(data = pattern) +
        geom_point(aes(x = t, y = y1), size = 2, col = "red") +
        geom_line(aes(x = t, y = fityln)) +
        scale_x_continuous(limits=c(0, n), breaks = seq(0, n, 2)) +
        scale_y_continuous(limits=c(0, 150), breaks = seq(0, 150, 10)) +
        labs(title = "Non-linear pattern\nfitted trend - exponential", x = "t", y = "result") +
        JT.theme
grid.arrange(p1, p2, nrow=1)
@
...
\caption{Polynomial trendline}
\label{fig:polytrend}
\setfloatalignment{b}
\end{marginfigure}

Other functions are possible. We can also try a recursive formula. To see which lag we should use we first calculate the partial autocorrelation (Figure~\ref{fig:pacftrend}). There is a clear correlation with lag1. Then we look for a linear regression between the time series and its lag1. To reconstruct the time series based on the recursive equation we need to specify the starting value $y_{1}$.

<<label=pacftrend, fig=TRUE, include=FALSE, echo=FALSE>>=
part <- data.frame(lag = pacf(pattern$y1, lag.max=10, plot = FALSE)$lag, 
                   acf = pacf(pattern$y1, lag.max=10, plot = FALSE)$acf)
p1 <- ggplot(data = part, aes(x = lag, y = acf)) +
        geom_segment(aes(xend = lag, yend = 0), size = 3) +
        scale_x_continuous(limits=c(0, 10), breaks = seq(0,10,1)) +
        scale_y_continuous(limits=c(0, 1), breaks = seq(0,1,0.1)) +
        labs(title = "Partial autocorrelation", x = "t", y = "result") +
        JT.theme
n <- 20
coef <- tidy(lm(y1[2:n] ~ y1[1:(n-1)], data = pattern))$estimate
pattern$yrec[1] <- 5
for(i in seq(2, n, 1)) {pattern$yrec[i] = coef[1] + coef[2]*pattern$yrec[i-1]}
p2 <- ggplot(data = pattern) +
        geom_point(aes(x = t, y = y1), size = 2, col = "red") +
        geom_line(aes(x = t, y = yrec)) +
        scale_x_continuous(limits=c(0, n), breaks = seq(0, n, 2)) +
        scale_y_continuous(limits=c(0, 150), breaks = seq(0, 150, 10)) +
        labs(title = "Non-linear pattern\nfitted trend - recursive", x = "t", y = "result") +
        JT.theme
grid.arrange(p1, p2, nrow=1)
@

\begin{marginfigure}
\includegraphics[width=1\textwidth]{PTS-pacftrend}
\caption{Trend fitted by a recursive relation}
\label{fig:pacftrend}
\setfloatalignment{b}
\end{marginfigure}
