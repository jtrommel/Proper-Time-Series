...
We start at 80\degree C, then we go to 100\degree C, then to 140\degree C and then we cool to 20\degree C etc. It will be obvious that this requires much more time for heating, cooling, reheating etc. than when we use the standard order. However, it is worth it because in this way we eliminate the possible influence of a steady increase in temperature on the measurement. In short: we set up our experiment is such a way that we the obeservations are \emph{independent}\index{independency} of each other (Figure~\ref{fig:independency2}-left). Plotting the measurement of pressure as a function of the moment in time that it was recorded, and assuming it takes 1 hour between two consecutive measurements, we get something like Figure~\ref{fig:independency2}-middle\sidenote{In reality the measurements will probably not be nicely evenly spaced}.
...
p2 <- ggplot(data = ideal) +
        ...
        labs(title = "Time series\nof experiment", x = "time (h)", y = "pressure (bar)") +
        scale_y_continuous(limits = c(0.9, 1.9)) +
        JT.theme
p3 <- ggplot(data = EUimm) +
        geom_point(aes(x = date, y = number), col = "red") +
