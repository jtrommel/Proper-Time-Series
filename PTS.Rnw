...
\begin{equation}
	\sigma^{2} = \frac{1}{n-1}\sum_{i=1}^{i=n}\left( y_{i} - \bar{y} \right)^{2}
\end{equation}
...
We can check if a time series has \emph{second order stationarity} using the concepts of \emph{autocovariance} and \emph{autocorrelation} of a time series. In general, when we have two random variables $Z_{1}$ and $Z_{2}$ the \emph{covariance} of these two is given by:
\begin{equation}
	cov(Z_{1},Z_{2})= E\left[ (Z_{1} - \mu_{Z_{1}})(Z_{2} - \mu_{Z_{2}}) \right] 
\end{equation}
