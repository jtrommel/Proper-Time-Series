...
% <<label=startgrafiekchance, fig=TRUE, include=FALSE, echo=FALSE>>=
% construct %>% mutate(add1 = rnorm(N, mean = mu, sd = sigma) + trend + season1 + season2) -> construct
% construct %>% mutate(add2 = rnorm(N, mean = mu, sd = sigma) + trend + season1 + season2) -> construct
% construct %>% mutate(add3 = rnorm(N, mean = mu, sd = sigma) + trend + season1 + season2) -> construct
% construct %>% mutate(add4 = rnorm(N, mean = mu, sd = sigma) + trend + season1 + season2) -> construct
% p1 <- ggplot(data = construct, aes(x = t)) +
%     geom_line(aes(y = add1), size = 0.75, color="grey") +
%     geom_line(aes(y = add2), size = 0.75, color="grey") +
%     geom_line(aes(y = add3), size = 0.75, color="grey") +
%     geom_line(aes(y = add4), size = 0.75, color="grey") +
%     geom_line(aes(y = add), size = 0.25, color="red") +
%     scale_y_continuous(limits= c(-25, 75)) +
%     labs(y="y") +
%     JT.theme
% p2 <- ggplot(data = construct, aes(x = t)) +
%     geom_point(aes(y = add1), size = 0.75, color="grey") +
%     geom_point(aes(y = add2), size = 0.75, color="grey") +
%     geom_point(aes(y = add3), size = 0.75, color="grey") +
%     geom_point(aes(y = add4), size = 0.75, color="grey") +
%     geom_point(aes(y = add), size = 0.25, color="red") +
%     scale_x_continuous(limits = c(499, 501), breaks = c(500)) +
%     scale_y_continuous(limits = c(0, 50)) +
%     labs(y="y") +
%     JT.theme
% p3 <- p1 + annotation_custom(ggplotGrob(p2), xmin = 350, xmax = 550, ymin = -30, ymax = 0)
% p3
% ggsave("PTS-startgrafiekchance.pdf")
% @
...
We constructed a stationary time series using the \emph{IID-principle}: \emph{Independent} samples taken from an \emph{Identical} \emph{Distribution}. 
...
\begin{equation}
	Cov(a,b)= \frac{1}{n-1}\sum_{i=1}^{i=n-1}(a_{i}-\bar{a})(b_{i}-\bar{b})
\end{equation}
...
$Cor(a,b)$ is dimensionless and its value of  will lie between -1 and 1. High absolute values of $Cor(a,b)$ indicate a strong correlation, values around 0 indicate no correlation at all.

\subsection{Using covariance and correlation in time series}

In time series we usually have only one set of $n$ values $y_{t}$ for each time point $t$. Suppose the values of this times series were given by a \emph{global} formula
\begin{equation}
	y_{t} = 5 + 0.2t
\end{equation}

For time point $t=1 \ldots 5$ the corresponding values of $y$ would be $(5.2, 5.4, 5.6, 5.8, 6.0)$. We could reconstruct the time series in another way by saying that:
\begin{equation}
	y_{t}=y_{t-1} + 0.2 \quad with \quad y_{1}=5.2
\end{equation}
This is a \emph{local} definition that tells us how one point is related to the previous point. The time series is correlated with itself. There is \emph{autocorrelation}.

If we want to calculate \emph{covariance} and \emph{correlation} we need pairs $(a_{i}, b_{i})$. Comparing each point with it's previous point we get $(n-1)=4$ pairs $(y_{2},y_{1}), (y_{3},y{2}), (y_{4},y{3}), (y_{5},y_{4})$. This we call \emph{auto-correlation Lag1}. But we can also compare each point with the data point two time points before. We get $(n-2)=3$ pairs $(y_{3},y_{1}), (y_{4},y{2}), (y_{5},y{3})$ (\emph{autocorrelation Lag2}). The \emph{sample autocovariance function Lagk} is defined by\sidenote{for all lags we still devide by the total length $n$ of the time series, and not by the number of pairs $(n-k)$ that are used for comparison. We also use everywhere $\bar{y}$, the average of the complete time series, and not the average of the partial time series. If the time series is long ($n$ large), this does not influence the result greatly. For small values of $n$ the effect can be substantial.}:
\begin{equation}
...
From this definition it is clear that $r_{0}=1$, which is what we expected: every time series is perfectly correlated with itself at Lag0. The \textbf{\textsf{R}} gives us a graph of the autocorrelation in a time series for lags=0 to 10. For our example where $y_{t}=5+0.2t$ ($n=1000$) we see in Figure~\ref{fig:autocorlin} perfect autocorrelation for all lags:

<<label=autocorlin, fig=TRUE, include=FALSE, echo=FALSE>>=
n <- 1000
autocorlin <- data.frame(t = seq(1,n), ylin = 0)
...
n <- 1000
autocornorm <- data.frame(t = seq(1,n), ynorm = rnorm(n, 0, 1))
...
<<>>=
# Ljung-Box test: stationary for p > 0.05
Box.test(stationary$ydice, type="Ljung-Box")
Box.test(stationary$ynorm, type="Ljung-Box")
# Augmented Dickey-Fuller: stationary for p < 0.05
adf.test(stationary$ydice)
adf.test(stationary$ynorm)
# Kwiatkowski-Phillips-Schmidt-Shin test: stationary for p > 0.05
kpss.test(stationary$ydice, null="Trend")
kpss.test(stationary$ynorm, null="Trend")
@
...
N.ext <- 25
stationary.ext <- data.frame(t = seq((N + 1),(N + N.ext)), ydice = mu.dice, lwr.ydice = low.lim.dice, upr.ydice = high.lim.dice, ynorm = mu.norm, lwr.ynorm = low.lim.norm, upr.ynorm = high.lim.norm)
p1 <- ggplot() +
  geom_line(data = stationary, aes(x = t, y = ydice), col = "red") +
  geom_line(data = stationary, aes(x = t, y = ynorm), col = "blue") +
  geom_line(data = stationary.ext, aes(x = t, y = ydice), color = "red", linetype = 2) +
  geom_line(data = stationary.ext, aes(x = t, y = ynorm), color = "blue", linetype = 2) +
  labs(title = "Prediction of stationary time series\n(assumption: normal distribution)", x = "t", y = "result") +
  geom_ribbon(data = stationary.ext, aes(x = t, ymin = lwr.ydice, ymax = upr.ydice), fill = "red", alpha = 0.3) +
  geom_ribbon(data = stationary.ext, aes(x = t, ymin = lwr.ynorm, ymax = upr.ynorm), fill = "blue", alpha = 0.3) +
...
<<>>=
mod.dice <- lm(ydice ~ t, data = stationary)
tidy(mod.dice)
mod.norm <- lm(ynorm ~ t, data = stationary)
tidy(mod.norm)
@

We can see that the p-values for the slope (\Sexpr{round(tidy(mod.dice)$p.value[2], 3)} and \Sexpr{round(tidy(mod.norm)$p.value[2], 3)}) are well above 0.05, so that there is no reason to reject the null-hypothesis that the slope = 0. This means that in both cases the predicted value in the future will be equal to the intercept value of the models (\Sexpr{round(tidy(mod.dice)$estimate[1], 3)} and \Sexpr{round(tidy(mod.norm)$estimate[1], 3)}). We can calculate a 95\% confidence interval for this model (Figure~\ref{fig:stationaryCI}-right).

<<label=stationaryCI, fig=TRUE, include=FALSE, echo=FALSE>>=
intercept.dice <- tidy(mod.dice)$estimate[1]
intercept.norm <- tidy(mod.norm)$estimate[1]
alpha <- 0.05
t0 <- qt((1 - alpha/2), N - 2)
stationary.pred <- data.frame(t = seq((N + 1),(N + N.ext)), 
                              ydice = intercept.dice,
                              lwr.dice = 0,
                              upr.dice = 0,
                              ynorm = intercept.norm,
                              lwr.norm = 0,
                              upr.norm = 0)
tbar <- mean(stationary$t)
SStt <- (N-1)*var(stationary$t)
stationary.pred$lwr.dice <- intercept.dice - t0*sqrt(var(stationary$ydice)*(1 + (1/N) + ((stationary.pred$t - tbar)^2)/SStt))
stationary.pred$upr.dice <- intercept.dice + t0*sqrt(var(stationary$ydice)*(1 + (1/N) + ((stationary.pred$t - tbar)^2)/SStt))
stationary.pred$lwr.norm <- intercept.norm - t0*sqrt(var(stationary$ynorm)*(1 + (1/N) + ((stationary.pred$t - tbar)^2)/SStt))
stationary.pred$upr.norm <- intercept.dice + t0*sqrt(var(stationary$ynorm)*(1 + (1/N) + ((stationary.pred$t - tbar)^2)/SStt))
p2 <- ggplot() +
  geom_line(data = stationary, aes(x = t, y = ydice), col = "red") +
  geom_line(data = stationary.pred, aes(x = t, y = ydice), col = "red", linetype = 2) +
  geom_ribbon(data = stationary.pred, aes(x = t, ymin = lwr.dice, ymax = upr.dice), fill = "red", alpha = 0.3) +
  geom_line(data = stationary, aes(x = t, y = ynorm), col = "blue") +
  geom_line(data = stationary.pred, aes(x = t, y = ynorm), col = "blue", linetype = 2) +
  geom_ribbon(data = stationary.pred, aes(x = t, ymin = lwr.norm, ymax = upr.norm), fill = "blue", alpha = 0.3) +
  labs(title = "Prediction of stationary time series\n(linear model with slope = 0)", x = "t", y = "result") +
  scale_y_continuous(limits=c(-1, 8)) +
  JT.theme
